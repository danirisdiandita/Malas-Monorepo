...
type UserQuery struct {
	...
	withRevenueCatIdentities *RevenueCatIdentityQuery
...
func (_q *UserQuery) QueryMealCalendarEntries() *MealCalendarEntryQuery {
...
// QueryRevenueCatIdentities chains the current query on the "revenue_cat_identities" edge.
func (_q *UserQuery) QueryRevenueCatIdentities() *RevenueCatIdentityQuery {
	query := (&RevenueCatIdentityClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRevenueCatIdentities: _q.withRevenueCatIdentities.Clone(),
...
func (_q *UserQuery) WithMealCalendarEntries(opts ...func(*MealCalendarEntryQuery)) *UserQuery {
	query := (&MealCalendarEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithRevenueCatIdentities tells the query-builder to eager-load the nodes that are connected to
// the "revenue_cat_identities" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithRevenueCatIdentities(opts ...func(*RevenueCatIdentityQuery)) *UserQuery {
	query := (&RevenueCatIdentityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRevenueCatIdentities = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [9]bool{
			...
			_q.withRevenueCatIdentities != nil,
		}
	)
	...
	if query := _q.withMealCalendarEntries; query != nil {
		if err := _q.loadMealCalendarEntries(ctx, query, nodes,
			func(n *User) { n.Edges.MealCalendarEntries = []*MealCalendarEntry{} },
			func(n *User, e *MealCalendarEntry) {
	...
	if query := _q.withRevenueCatIdentities; query != nil {
		if err := _q.loadRevenueCatIdentities(ctx, query, nodes,
			func(n *User) { n.Edges.RevenueCatIdentities = []*RevenueCatIdentity{} },
			func(n *User, e *RevenueCatIdentity) {
				n.Edges.RevenueCatIdentities = append(n.Edges.RevenueCatIdentities, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadMealCalendarEntries(ctx context.Context, query *MealCalendarEntryQuery, nodes []*User, init func(*User), assign func(*User, *MealCalendarEntry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mealcalendarentry.FieldUserID)
	}
	query.Where(predicate.MealCalendarEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MealCalendarEntriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadRevenueCatIdentities(ctx context.Context, query *RevenueCatIdentityQuery, nodes []*User, init func(*User), assign func(*User, *RevenueCatIdentity)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(revenuecatidentity.FieldUserID)
	}
	query.Where(predicate.RevenueCatIdentity(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RevenueCatIdentitiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
