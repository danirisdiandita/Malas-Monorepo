...
func HandleImport(token, debugDir, authURL, webhookSecret, tikTokActorURL, facebookReelsActorURL, facebookPostsActorURL, instagramActorURL, instagramReelsActorURL, pinterestActorURL, youtubeActorURL, youtubeTranscriptActorURL, webActorURL string, pipeline *Pipeline) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		} else if contentType == PinterestPin {
			source, actorURL = "pinterest", pinterestActorURL
			actorInput = pinterestActorInput(redirected.String())
...
func ParseLinkContentType(raw string) (LinkContentType, error) {
	...
	if isPinterestHost(u.Hostname()) && strings.HasPrefix(strings.ToLower(u.Path), "/pin/") {
		return PinterestPin, nil
	}
...
func isPinterestHost(host string) bool {
	host = strings.ToLower(strings.TrimSuffix(host, "."))
	return host == "pinterest.com" || strings.HasSuffix(host, ".pinterest.com") || host == "pin.it"
}
...
func pinterestActorInput(url string) map[string]any {
	return map[string]any{
...
func youtubeActorInput(url string) map[string]any {
	return map[string]any{
		"aiVideoDescription": false, "aiVideoSummary": false,
		"hasCC": false, "hasLocation": false, "hasSubtitles": false,
		"is360": false, "is3D": false, "is4K": false, "isBought": false, "isHD": false,
		"isHDR": false, "isLive": false, "isVR180": false, "maxResultStreams": 0,
		"maxResults": 1, "maxResultsShorts": 0, "preferAutoGeneratedSubtitles": false,
